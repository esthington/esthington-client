...
export function EditInvestmentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (id) {
        ...
        if (investment) {
          // Populate form data from investment
          setFormData({
            propertyId:
            ...
            minInvestment: investment.minimumInvestment.toString(),
            maxInvestors: investment.maxInvestors,
            ...
            newAmenity: "",
            investmentPlans: [],
            durations: investment.durations,
  ...
  }, [formData.propertyId, filteredProperties]);
  ...
  useEffect(() => {
    if (formData.investmentPeriod && formData.startDate) {
      ...
      endDate.setMonth(
        startDate.getMonth() + Number.parseInt(formData.investmentPeriod)
      );
      setFormData((prev) => ({
        ...prev,
  ...
  const handlePlanChange = (index: number, field: string, value: string) => {
    setFormData((prev) => {
      ...
      if (index === 0) {
        if (
          field === "minAmount" &&
          Number(value) < Number(prev.minInvestment)
        ) {
          toast.error(
            "Plan 1 minimum amount cannot be less than the base minimum investment"
          );
          return prev;
        }
        ...
        if (field === "returnRate" && Number(value) < Number(prev.returnRate)) {
          toast.error(
            "Plan 1 return rate cannot be less than the base return rate"
          );
          return prev;
        }
      }
  ...
  const handleDurationChange = (
    ...
    setFormData((prev) => {
      ...
      if (
        index === 0 &&
        field === "months" &&
        Number(value) < Number(prev.investmentPeriod)
      ) {
        toast.error(
          "First duration cannot be less than the base investment period"
        );
        return prev;
      }
  ...
  const validateForm = () => {
    ...
    if (
      !formData.returnRate ||
      isNaN(Number(formData.returnRate)) ||
      Number(formData.returnRate) <= 0
    ) {
    ...
    if (
      !formData.investmentPeriod ||
      isNaN(Number(formData.investmentPeriod)) ||
      Number(formData.investmentPeriod) <= 0
    ) {
    ...
    if (
      !formData.minInvestment ||
      isNaN(Number(formData.minInvestment)) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      ...
      if (firstErrorKey === "propertyId") {
        ...
        ["startDate", "endDate", "returnType", "payoutFrequency"].includes(
          firstErrorKey
        )
      ) {
    ...
    try {
      ...
      apiFormData.append(
        "investmentPlans",
        JSON.stringify(formData.investmentPlans)
      );
      ...
      const investmentId = await updateInvestment(id, apiFormData);
  ...
  const handleFileUpload = (
    ...
    setFormData((prev: any) => {
      if (field === "documents") {
        // Only treat as files for the 'documents' field
        const prevDocs = Array.isArray(prev.documents) ? prev.documents : [];
        return {
          ...prev,
          documents: [...prevDocs, ...files],
        };
      }
      // Add similar logic for other file fields if needed
      return prev;
    });
  };
  ...
  const removeFile = (index: number, field: string) => {
    setFormData((prev) => {
      if (field === "documents") {
        const updatedFiles = Array.isArray(prev.documents) ? [...prev.documents] : [];
            ...
            <div className="flex gap-2">
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <Save className="mr-2 h-4 w-4" /> Update Investment
                  </>
                )}
              </Button>
            </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <p className="text-sm text-slate-500 dark:text-slate-400">
                            Edit existing plans to attract different types of
                            investors
                          </p>
                          <Button
                            type="button"
