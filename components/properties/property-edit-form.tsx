...
export default function PropertyEditForm({
  ...
  const [formData, setFormData] = useState({
    ...
    galleryUrls: [] as Array<string>,
    planFile: null as File | null,
    planFileUrl: "",
    documents: [] as Array<File>,
    documentUrls: [] as Array<string>,
  });
  ...
  useEffect(() => {
    if (companies.length > 0 && formData.companyId) {
      // Check if the company exists in the loaded companies
      const companyExists = companies.some((c) => c._id === formData.companyId);

      if (!companyExists) {
        console.warn(
          "Company ID not found in companies list:",
          formData.companyId
        );
        // If needed, you could set a default company here
        // setFormData(prev => ({ ...prev, companyId: companies[0]?._id || "" }));
      } else {
        console.log("Company found and selected:", formData.companyId);
      }
    }
  }, [companies, formData.companyId]);
    ...
    const fetchPropertyData = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          totalPlots: propertyData.totalPlots?.toString() || "",
          type: propertyData.type || "Land",
          status: propertyData.status || "Available",
          featured: propertyData.featured || false,
          companyId: propertyData.companyId._id || "",
          amenities: propertyData.amenities || [],
          plots:
            propertyData.plots?.map((plot: any) => ({
              id: plot._id || String(Math.random()),
              _id: plot._id,
              plotId: plot.plotId || "",
              size: plot.size || "",
              price:
          ...
          thumbnail: null,
          thumbnailUrl: propertyData.thumbnail || "",
          gallery: [],
          galleryUrls: propertyData.gallery || [],
          planFile: null,
          planFileUrl: propertyData.planFile || "",
          documents: [],
          documentUrls: propertyData.documents || [],
        });
      } catch (error) {
  ...
  // Log selected company when companies are loaded
  useEffect(() => {
    if (companies.length > 0 && formData.companyId) {
      ...
      console.log(
        "Selected company:",
        selectedCompany ? selectedCompany.name : "Not found"
      );
  ...
  const handleFileUpload = (
    ...
    if (type === "thumbnail" && files.length > 0) {
      setFormData((prev) => ({
        ...prev,
        thumbnail: files[0],
        thumbnailUrl: "", // Clear the URL since we have a new file
      }));
      ...
      setFormData((prev) => ({
        ...prev,
        planFile: files[0],
        planFileUrl: "", // Clear the URL since we have a new file
      }));
  ...
  const handleRemoveGalleryImage = (index: number, isUrl = false) => {
    if (isUrl) {
      const updatedGalleryUrls = [...formData.galleryUrls];
      updatedGalleryUrls.splice(index, 1);
      setFormData((prev) => ({ ...prev, galleryUrls: updatedGalleryUrls }));
    } else {
  ...
  const handleRemoveDocument = (index: number, isUrl = false) => {
    if (isUrl) {
      const updatedDocumentUrls = [...formData.documentUrls];
      updatedDocumentUrls.splice(index, 1);
      setFormData((prev) => ({ ...prev, documentUrls: updatedDocumentUrls }));
    } else {
  ...
  const validateForm = () => {
    ...
    // Validate thumbnail - for edit form, we allow existing thumbnailUrl
    ...
    try {
      ...
      const propertyData = {
        ...
        plots: formData.plots.map((plot) => ({
          ...(plot._id ? { _id: plot._id } : {}),
          plotId: plot.plotId,
          size: plot.size,
          price: Number(plot.price),
          status: plot.status,
        })),
        // Include existing URLs that should be kept
        thumbnailUrl: formData.thumbnailUrl,
        galleryUrls: formData.galleryUrls,
        planFileUrl: formData.planFileUrl,
      ...
      // Append gallery images if new ones were selected
      formData.gallery.forEach((image) => {
        formDataObj.append("gallery", image);
      });

      // Append plan file if a new one was selected
      if (formData.planFile) {
        formDataObj.append("planFile", formData.planFile);
      }

      // Append documents if new ones were selected
      formData.documents.forEach((doc) => {
        formDataObj.append("documents", doc);
      });

      const result = await updateProperty(propertyId, formDataObj);

      if (result) {
        toast.success("Property updated successfully", {
          description: `Changes to ${result.title} have been saved.`,
        });
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {isLoadingCompanies ? (
                      <div className="h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 flex items-center px-3">
              ...
              <div className="p-6">
                ...
                <Tabs defaultValue="thumbnail" className="w-full">
                          ...
                          <img
                            src={
                              formData.thumbnail
                                ...
                                : formData.thumbnailUrl || "/placeholder.svg"
                            }
                          ...
                          >
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-between p-3">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleRemoveGalleryImage(index, true)
                                }
                                className="h-8 w-8 p-0 rounded-full"
                              >
                                <Trash2 className="h-4 w-4" />
                  ...
                  <TabsContent value="plan" className="space-y-4">
                    ...
                    {formData.planFile || formData.planFileUrl ? (
                          ...
                          <div className="flex-1">
                            <p className="text-slate-900 dark:text-white font-medium">
                              {formData.planFile
                                ...
                                : "Property Plan Document"}
                            </p>
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                              {formData.planFile
                                ? (formData.planFile.size / 1024).toFixed(2) +
                                  " KB"
                                : "Current plan file"}
                            </p>
                          </div>
                    ...
                    >
                      ...
                      <div className="flex items-center mb-4">
                        ...
                        <h3 className="font-medium text-slate-900 dark:text-white">
                          Plot {index + 1} {plot._id ? "(Existing)" : "(New)"}
                        </h3>
                      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1 ml-[52px]">
              Update property information for {formData.title}
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-4 mb-6 shadow-sm">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Update all required information to maintain a comprehensive
                property listing
              </p>
            </div>
          </div>
